...
- **`stat_meta` completeness.** `StatMeta` is REQUIRED for any user-facing stat — any stat that should
  ...
  does NOT assert every `Stat` has meta — only structure/validity of the entries present.
