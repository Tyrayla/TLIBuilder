...
def _net_life_at(source, condition_state, life_pct, restoration_inputs, rates) -> float:
    cs = dict(condition_state)
    cs["current_life_pct"] = life_pct
    cons = calculate_consumption(source, condition_state=cs, rates=rates)
...
def solve_steady_life_pct(source, condition_state, restoration_inputs, rates) -> float:
    """Bisect the Life % where net_life = 0; quantized to LIFE_PCT_QUANTUM in [0, 100]."""
    if _net_life_at(source, condition_state, 100.0, restoration_inputs, rates) >= 0.0:
        return 100.0
    if _net_life_at(source, condition_state, 0.0, restoration_inputs, rates) <= 0.0:
        return 0.0
    lo, hi = 0.0, 100.0   # f(lo) > 0 (recovering), f(hi) < 0 (draining)
    for _ in range(9):    # 100 / 2^9 ≈ 0.2 % precision
        mid = 0.5 * (lo + hi)
        if _net_life_at(source, condition_state, mid, restoration_inputs, rates) >= 0.0:
            lo = mid
        else:
            hi = mid
    mid = 0.5 * (lo + hi)
