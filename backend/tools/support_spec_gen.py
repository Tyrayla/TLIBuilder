...
model mapping the resolver will implement, plus the Attack/Spell tag-gate, so the owner can verify each
support before coding.

Run:  py -3.12 backend/tools/support_spec_gen.py   (reads existing doc to PRESERVE owner annotations,
...
Tag-gate (owner rule 2026-06-10): gate on Attack/Spell ONLY — a support tagged Spell (and not Attack)
...
RULES: list[tuple[str, str, str, str]] = [
    ...
    (r"additional\s*ailment\s*damage\s*for the supported", N, "ailment_dmg_additional?", "defer until ailment DPS modeled (owner)"),
    (r"additional\s*damage over time\s+(for|against)", N, "dot_dmg_additional?", "defer until DoT modeled (owner)"),
    ...
    (r"damage over time against enemies with\s*max\s*affliction", N, "dot_dmg_additional?", "defer until DoT modeled (owner)"),
    (r"more damage to enemies with more life", D, "erosion_dmg_additional", "scales with enemy Life — needs enemy-life model"),
    (r"additional damage for the supported skill when it lands a critical strike", N, "dmg_additional", "crit-weighted: ×(1 + crit_chance·x) — model this (owner)"),
    (r"types of\s*elemental\s*damage, the next use", D, "elemental_dmg_additional", "next-use conditional — defer"),
    (r"damage increase per wave", D, "barrage_dmg_per_wave_inc", "wave mechanic — capture as stat now (owner)"),
...
def _load_annotations() -> dict[str, str]:
    """Owner notes keyed by support name, from docs/_support_annotations.json (edited by the owner,
...
def main() -> None:
    ...
    out = ["# Standard Support Skill — Modeling Spec (per-support)",
           ...
           "from SS12 data; owner annotations preserved in the **📝** column. **Goal: model each support "
    ...
    # New-infrastructure checklist (curated from owner annotations + decisions).
    out += [
    ...
    with open(_OUT, "w", encoding="utf-8") as f:
        f.write("\n".join(out) + "\n")
    print(f"wrote {_OUT} — {len(ann_by_support)} owner annotations attached")
