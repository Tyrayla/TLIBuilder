...
describe('StatsErrorBanner', () => {
  it('renders nothing when there is no error', () => {
    ...
    act(() => { renderer = TestRenderer.create(<StatsErrorBanner message="" />) })
  ...
  it('renders the engine message prominently when a calculation fails', () => {
    ...
    let renderer!: TestRenderer.ReactTestRenderer
    act(() => { renderer = TestRenderer.create(<StatsErrorBanner message={message} />) })
    expect(renderer.root.findAllByProps({ role: 'alert' })).toHaveLength(1)
    const text = JSON.stringify(renderer.toJSON())
    expect(text).toContain(message)
    expect(text.toLowerCase()).toContain('calculation failed')
  })
})
